...
@Client.on_message(
    command(["musicp", f"musicp@{BOT_USERNAME}"])
    & ~filters.edited
    & ~filters.bot
    & ~filters.private
)
...
@Client.on_message(command(["Johnnyplay", f"Johnnyplay@{BOT_USERNAME}"]) & other_filters)
async def Johnnyplay(_, message: Message):
